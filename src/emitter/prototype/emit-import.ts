...
export function _emitImport(this: IEmitterInternal, ir: IImportIR): void {
  const { source, specifiers, isTypeOnly } = ir;
  ...
  // For full statement type imports, add a special marker
  if (isTypeOnly) {
    this.context.imports.addImport(source, 'statement-type:true');
  }
  ...
  for (const spec of specifiers) {
    ...
    // For type-only imports, prefix with 'type:'
    ...
    if (spec.type === 'ImportDefaultSpecifier') {
      ...
      // Add type prefix for inline type imports
      if (spec.isTypeOnly) {
        importName = `type:${importName}`;
      }
    }
