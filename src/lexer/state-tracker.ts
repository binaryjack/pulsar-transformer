...
export class StateTransitionTracker {
  ...
  tracePosition(pos: number): string {
    ...
    const lines: string[] = [`🎯 State transitions near position ${pos}:`];

    nearby.forEach((t) => {
      const marker = t.position === pos ? '👉' : '  ';
      const depthInfo = t.depth ? ` [jsx:${t.depth.jsx}, tpl:${t.depth.template}]` : '';
  ...
  analyzePatterns(): {
    ...
    const problematicTransitions = this.transitions.filter(
      (t) =>
        // Deep nesting
