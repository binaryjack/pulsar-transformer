...
export class LexerDebugger {
  ...
  updatePeakDepths(jsx: number, template: number, expression: number): void {
    ...
    this.performanceData.peakDepths.template = Math.max(this.performanceData.peakDepths.template, template);
    this.performanceData.peakDepths.expression = Math.max(this.performanceData.peakDepths.expression, expression);
  }
  ...
  compareSnapshots(before: LexerSnapshot, after: LexerSnapshot): string[] {
    ...
    if (before.position !== after.position) {
      changes.push(`Position: ${before.position} → ${after.position} (+${after.position - before.position})`);
    }
  ...
  printSnapshot(snapshot: LexerSnapshot, label?: string): void {
    ...
    console.log(`   Depths: jsx=${snapshot.depths.jsx}, template=${snapshot.depths.template}, expression=${snapshot.depths.expression}`);
    
    if (snapshot.flags.justExitedJSXTextForBrace) {
      console.log(`   Flags: justExitedJSXTextForBrace=true`);
    }
    
    console.log(`   Context: ...${snapshot.contextWindow.before}[${snapshot.contextWindow.current}]${snapshot.contextWindow.after}...`);
  ...
  getPerformanceReport(): string {
    ...
    if (this.performanceData.slowestTokens.length > 0) {
      ...
      this.performanceData.slowestTokens.forEach((token, i) => {
        lines.push(`     ${i + 1}. ${token.type}:"${token.value}" (${token.time.toFixed(3)}ms at pos ${token.position})`);
      });
    }
...
export class LexerDebugSession {
  ...
  step(): boolean {
    ...
    try {
      ...
      console.log(`\n🔄 Step ${this.stepCount}:`);
      this.debugTool.printComparison(`step-${this.stepCount}-before`, `step-${this.stepCount}-after`);
