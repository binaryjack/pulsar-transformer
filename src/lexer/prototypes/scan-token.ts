...
Lexer.prototype.scanToken = function (this: ILexer): void {
  ...
  if ((char === '"' || char === "'") && this.jsxDepth > 0 && currentState === LexerStateEnum.Normal) {
  ...
  if (this.warningSystem) {
    const isUnsupported = this.warningSystem.checkUnsupportedFeatures(this, char, this.source.slice(Math.max(0, this.pos - 10), this.pos));
    if (isUnsupported && this.recoveryController?.shouldContinueOnError(new Error('Unsupported feature'))) {
